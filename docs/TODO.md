...
- [x] Widen `ci.yml` to run on every branch push (previously only `main` and `v*` tags),
      drop the tag trigger there now that tag pushes are `release.yml`'s job, and drop
      the `pull_request` trigger so a same-repo PR branch runs CI once per push instead
      of twice (`docs/adr-phase-9.md` ADR-0044)
